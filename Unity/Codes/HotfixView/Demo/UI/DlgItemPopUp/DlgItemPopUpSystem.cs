...
namespace ET
{
    ...
    public static class DlgItemPopUpSystem
    {
        ...
        {
            self.RemoveUIScrollItems(ref self.ScrollItemEntryDict);
        }


        public static void OnEntryLoopHandler(this DlgItemPopUp self, Transform transform, int index)
        {
            Scroll_Item_Entry scrollItemEntry = self.ScrollItemEntryDict[index].BindTrans(transform);
            ...
            scrollItemEntry.E_EntryNameTextMeshProUGUI.text = PlayerNumericConfigCategory.Instance.Get(entry.Key).Name + ":";
            bool isPrcent = PlayerNumericConfigCategory.Instance.Get(entry.Key).isPrecent > 0;
            scrollItemEntry.E_EntryValueTextMeshProUGUI.text = "+" + (isPrcent ? $"{(entry.Value / 10000.0f).ToString("0.00")}%" : entry.Value.ToString());
        }
        ...
        public static void RefreshInfo(this DlgItemPopUp self, Item item, ItemContainerType itemContainerType)
        {
            ...
            self.View.E_NameTextMeshProUGUI.text = item.Config.Name;
            self.View.E_DescTextMeshProUGUI.text = item.Config.Desc;
            self.View.E_PriceTextMeshProUGUI.text = item.Config.SellBasePrice.ToString();
            ...
            {
                self.View.E_ScoreTextMeshProUGUI.text = item.GetComponent<EquipInfoComponent>().Score.ToString();
                int count = item.GetComponent<EquipInfoComponent>().EntryList.Count;
                self.AddUIScrollItems(ref self.ScrollItemEntryDict, count);
